...
openssl genrsa -des3 -out ${USERS_DIR}/${USERNAME}.key -passout pass:$Client_PASSWORD 1024
...
openssl req -passin pass:$Client_PASSWORD -config $SSL_DIR/${CA_NAME}/openssl.cnf -new -key ${USERS_DIR}/${USERNAME}.key -out ${USERS_DIR}/${USERNAME}.csr -subj "${SSL_SUBJ}"
...
openssl x509 -passin pass:$CA_PASSWORD -req -days 1095 -in ${USERS_DIR}/${USERNAME}.csr -CA $SSL_CERTS_DIR/ca.crt -CAkey $SSL_PRIVATE_DIR/ca.key -CAserial $SSL_DIR/${CA_NAME}/serial -CAcreateserial -out ${USERS_DIR}/${USERNAME}.crt

echo "making p12 file"
#browsers need P12s (contain key and cert)
openssl pkcs12 -passin pass:$Client_PASSWORD -passout pass:$ClientExport_PASSWORD -export -clcerts -in ${USERS_DIR}/${USERNAME}.crt -inkey ${USERS_DIR}/${USERNAME}.key -out ${USERS_DIR}/${USERNAME}.p12
